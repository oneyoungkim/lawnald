...
async def websocket_endpoint(websocket: WebSocket, lawyer_id: str, client_id: str, role: str):
    print(f"WebSocket Connection Attempt: {lawyer_id}, {client_id}, {role}")
    try:
        await chat_manager.connect(websocket, lawyer_id, client_id, role)
        print(f"WebSocket Connected: {lawyer_id}, {client_id}, {role}")
    except Exception as e:
        print(f"WebSocket Connection Failed: {e}")
        # Allow connection to close gracefully if manager fails, or just re-raise
        # raise e 
...
@app.get("/api/test_chat_route")
def test_chat_route():
    return {"message": "Chat route exists"}

@app.websocket("/ws/test")
async def ws_test(websocket: WebSocket):
    await websocket.accept()
    await websocket.send_text("Hello from WS")
    await websocket.close()

async def ws_test_manual(websocket: WebSocket):
    await websocket.accept()
    await websocket.send_text("Manual WS")
    await websocket.close()

app.add_websocket_route("/ws/manual", ws_test_manual)
...
def get_monthly_stats():
    ...
    # 2. Consultations
    current_consults = [c for c in CONSULTATIONS_DB if c.get("created_at") >= month_ago.strftime("%Y-%m-%d")]
...
def create_consultation(request: ConsultationCreateRequest):
    # Analyze text
...
async def signup_lawyer(
    ...
    save_lawyers_db(LAWYERS_DB)
    return {"message": "Signup successful", "lawyer_id": new_lawyer["id"]}
...
@app.get("/api/admin/magazine/all")
def get_all_magazine_content():
    """List all content items from all lawyers for admin management."""
    all_content = []
    for lawyer in LAWYERS_DB:
        for item in lawyer.get("content_items", []):
            all_content.append({
